...
def find_best_split(feature_vector, target_vector):
    ...
    * За наличие в функции циклов балл будет снижен. Векторизуйте! :)
    ...
    results = {}
    # sorted_features = feature_vector.sort()
    sorting_ord = np.argsort(feature_vector)
    sorted_features = feature_vector[sorting_ord]
    sorted_target = target_vector[sorting_ord]
    unique_vals = np.unique(sorted_features)
    thresholds = (unique_vals[:-1] + unique_vals[1:]) / 2
    classes_sum = np.cumsum(sorted_target)
    if len(thresholds) == 0:
        return thresholds, np.array([]), None, None

    left_tree = np.searchsorted(sorted_features, thresholds, side='right')
    right_tree = len(sorted_features) - left_tree
    left_tree_classes = classes_sum[left_tree - 1]
    right_tree_classes = np.sum(sorted_target) - left_tree_classes

    gini_left_tree = 1 - ((left_tree - left_tree_classes) /
                          left_tree) ** 2 - (left_tree_classes / left_tree) ** 2
    gini_right_tree = 1 - ((right_tree - right_tree_classes) /
                           right_tree) ** 2 - (right_tree_classes / right_tree) ** 2
    gini = - (left_tree / len(sorted_features)) * gini_left_tree - \
        (right_tree / len(sorted_features)) * gini_right_tree

    threshold_best = thresholds[gini == np.max(gini)][0]

    return thresholds, gini, threshold_best, np.max(gini)
    ...
    * ограничения max_depth, min_samples_split, min_samples_leaf (как в sklearn по смыслу)
    ...
    def _fit_node(self, sub_X, sub_y, node, depth):
        ...
        self._fit_node(sub_X[np.logical_not(split)],
                       sub_y[np.logical_not(split)], node["right_child"], depth+1)
    ...
    def _predict_node(self, x, node):
        ...
        if feature_type == "categorical":
            if x[feature_ind] in node["categories_split"]:
                return self._predict_node(x, node["left_child"])
            else:
                return self._predict_node(x, node["right_child"])
